...
export default class LaunchControlTrackContainer extends React.Component {
  render() {
    ...
    let trackElements = [ 0, 1, 2, 3, 4, 5, 6, 7 ].map((track) => {
      let knob1data = { track, index: 0, value: data.params[track], active: data.activeKnob === track };
      let knob2data = { track, index: 1, value: data.params[track + 8], active: data.activeKnob === track + 8 };
